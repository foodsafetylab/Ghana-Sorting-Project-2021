...
AFfarmsweight <- ggplot(data = Ghana2) + 
  geom_boxplot(aes(x= Sample_type, y= perc_weight2 )) + labs(title = "Percentage of Sample Mass in Accept and Reject Stream after Sorting", x = "Sorted Sample", y = "Mass Sorted (%)") + theme(legend.position = c(0.2, 0.8)) + coord_cartesian(ylim = c(0, 1)) 
...
AFfarmstoxinremoval <- ggplot(data = Ghana3) + 
  geom_boxplot(aes(x= Sample_type, y= Perc_toxin_removed_AF)) + labs(title = "Percentage of Total Aflatoxin Fraction Removed by Sorting", x = "Sample Stream", y = "Aflatoxin removed in Reject Samples (%)") + theme(legend.position = c(0.2, 0.8)) + coord_cartesian(ylim = c(0, 1)) 

#+ annotate("text", x = 0.6, y = 3, label = paste ("Mean ==", mean_d1), parse = TRUE) + annotate("text", x = 0.6, y = 2.8, label = paste ("Mode ==", mode_d1), parse = TRUE)


plot (AFfarmstoxinremoval)
```
...
FMfarmstoxinremoval <- ggplot(data = Ghana4) + 
  geom_boxplot(aes(x= Sample_type, y= Perc_toxin_removed_FM)) + labs(title = "Percentage of Total Fumonisin Fraction Removed by Sorting", x = "Sample Stream", y = "Fumonisin removed in Reject Samples (%)") + theme(legend.position = c(0.2, 0.8)) + coord_cartesian(ylim = c(0, 1)) 
